...
int main() {
    ...
    string PhienBan = "2.1.120126", pass = "cudem1981", TacGia = "NguyenPhuoc";
...
ChuKy:
    ...
    if (a == "exit") {
        print << "\n";
        for (int i = 4; i!= 0; i--) {
            print << "\rChương trình sẽ tắt sau: " << i-1 << " " << std::flush;
            wait(1s);
        }
        print << "\rChương trình đã tắt!       " << std::flush;
        return 0;
    } else if (a == "game") {
        string b;
        scan >> b;
        if (b == "chanle") {
            ...
            print << "\n";
            for (int i = 61; i != 0 ; i--) {
    // Thêm std::flush để ép dữ liệu hiện ra màn hình ngay lập tức
    print << "\rThời gian còn lại: " << i - 1 << "   " << std::flush;
    wait(1s);
}
            cout << "\r";
    ...
    wait(1s);
            ...
            for (int i = 4; i != 0; i--) {
                cout << "\r";
                print << "Thời gian còn lại: " << i - 1 << " ";
                wait(1s);
            }
            cout << "\r";
            print << "---Dự Đoán Tên Người Yêu Tương Lai----------------";
